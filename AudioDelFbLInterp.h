#ifndef AUDIODELAY_FEEDBACK_LINTERP_H_
#define AUDIODELAY_FEEDBACK_LINTERP_H_

#include "Arduino.h"
#include "mozzi_utils.h"

/** Audio delay line with linear interpolation and feedback for comb filter, flange, chorus and short echo effects.
...
milliseconds. AudioDelFbLInterp uses int sized cells to accomodate the higher
...
with the input, do it in your sketch. AudioDelFbLInterp uses more processing
than a plain AudioDelay or an AudioDelayFeedback, but gives smoother transitions
between delay times.
*/

template <unsigned int NUM_BUFFER_SAMPLES>
class AudioDelFbLInterp
{
...
public:
	/** Constructor.
	*/
	AudioDelFbLInterp(): write_pos(0), _delaytime_cells(0), _feedback_level(0)
	{}
	...
	*/
	AudioDelFbLInterp(unsigned int delaytime_cells): write_pos(0), _feedback_level(0), _delaytime_cells(delaytime_cells)
	{}
	...
	*/
	AudioDelFbLInterp(unsigned int delaytime_cells, char feedback_level): write_pos(0), _delaytime_cells(delaytime_cells), _feedback_level(feedback_level)
	{}
...
#endif        //  #ifndef AUDIODELAY_FEEDBACK_LINTERP_H_
