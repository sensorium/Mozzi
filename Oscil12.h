...
public:
	...
	folder.*/
	Oscil12(const int16_t * TABLE_NAME):table(TABLE_NAME)
	{}
	...
	*/
	...
	{
		incrementPhase();
		return (int16_t)CONSTTABLE_READ_WORD(table + (((phase_fractional+(phmod_proportion * NUM_TABLE_CELLS))>>OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
	}
...
*/
	...
	*/
	...
	{
		return (int16_t)CONSTTABLE_READ_WORD(table + (index & (NUM_TABLE_CELLS - 1)));
	}
...
static const uint16_t ADJUST_FOR_NUM_TABLE_CELLS = (NUM_TABLE_CELLS<2048) ? 8 : 0;
	...
	{
#ifdef OSCIL_DITHER_PHASE
		return CONSTTABLE_READ_WORD(table + (((phase_fractional + ((int)(xorshift96()>>16))) >> OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
#else
		return CONSTTABLE_READ_WORD(table + ((phase_fractional >> OSCIL_F_BITS) & (NUM_TABLE_CELLS - 1)));
		//return (int8_t)CONSTTABLE_READ_WORD(table + (((phase_fractional >> OSCIL_F_BITS) | 1 ) & (NUM_TABLE_CELLS - 1))); odd phase, attempt to reduce frequency spurs in output
#endif
	}
