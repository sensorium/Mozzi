/*  Plays a fluctuating ambient wash using pairs
    ...
    For this sketch to run better on 16MHz Ateml boards in Arduino versions later than 1.5,
    edit platform.txt (on OSX you can find it by searching in /Users/your_name/Library/Arduino15).
    Search and replace -Os with -O2.  Save.  
    This tells the compiler to optimise for speed, instead of size.
    However, some unwanted ticking sounds will still be audible, and it still runs faster when compiled with Arduino 1.0.5.
